...
<%
	...
	int pageLimit = 8;
	int aid = key.contains(",") ? 0 : Integer.parseInt(key);
	// 获取艺人信息
	...
	<body onload='start()' onunload='destoryMP()' bgcolor='<%if(isOnline.equals("y")){%>transparent<%} else{%>#444444<%}%>'>
		...
		<div id='realDis' style='position:absolute;width:<%=pageW %>px;height:<%=pageH %>px;overflow:hidden;display:none'>
			...
			<div id='BigDiv' style='position:absolute;width:<%=pageW %>px;height:<%=pageH  %>px;left:0px;top:0px'>
				...
				<img id='artist_100004' src='images/application/pages/artist/playAll.png' style='position:absolute;width:153px;height:55px;left:156px;top:468px;z-index:1' />
				<img class='btn_focus_hidden' id='f_artist_100004' src='images/application/pages/artist/focus/f_playAll.png' style='position:absolute;width:153px;height:55px;left:156px;top:468px;z-index:2' />
				<img id='artist_100005' src='images/application/pages/artist/fashion.png' style='position:absolute;width:153px;height:55px;left:156px;top:528px;z-index:1' />
				<img class='btn_focus_hidden' id='f_artist_100005' src='images/application/pages/artist/focus/f_fashion.png' style='position:absolute;width:153px;height:55px;left:156px;top:528px;z-index:2' />
				<div style='position:absolute;width:235px;height:30px;left:115px;top:390px;font-size:28px;color:#FFFFFF;text-align:center;z-index:2'><%=gabr.get("cname").toString().replaceAll("'", "\\\\'") %></div>
				<div style='position:absolute;width:355px;height:30px;left:55px;top:425px;font-size:20px;color:#B2AFAA;text-align:center;z-index:2'><%=StringUtils.isNotBlank(gabr.get("sname").toString()) ? gabr.get("sname").toString().replaceAll("'", "\\\\'") : "────" %></div>
				<div id='rightDiv_mar' style='position:absolute;width:1280px;height:720px;left:25px;top:0px'>
					...
					<div style='position:absolute;width:50px;height:30px;left:575px;top:206px;font-size:18px;color:#FFFFFF;z-index:2'>热度</div>
					<div style='position:absolute;width:150px;height:30px;left:630px;top:200px;font-size:30px;color:#FFFFFF;z-index:2'><%=disNum %></div>
					...
					<img class='btn_focus_hidden' id='f_artist_100006' src='images/application/pages/search/commonly/focus/f_s.png' style='position:absolute;width:36px;height:36px;left:1218px;top:373px;z-index:2' />
					<img class='btn_focus_hidden' id='f_artist_100007' src='images/application/pages/search/commonly/focus/f_x.png' style='position:absolute;width:36px;height:36px;left:1218px;top:492px;z-index:2' /><%for(int i = 1; i <= pageLimit; i++){ int x = 430; int y = 222;%>
					...
					<img id='artist_<%=i %>' src='images/application/pages/search/song/play.png' style='position:absolute;width:58px;height:58px;left:<%=x + 563 %>px;top:<%=(i - 1) * 52 + y + 13 %>px;z-index:3;visibility:hidden' />
					<img class='btn_focus_hidden' id='f_artist_<%=i %>' src='images/application/pages/search/song/focus/f_play.png' style='position:absolute;width:58px;height:58px;left:<%=x + 563 %>px;top:<%=(i - 1) * 52 + y + 13 %>px;z-index:4' />
					<img id='artist_<%=i + pageLimit %>' src='images/application/pages/search/song/collect.png' style='position:absolute;width:58px;height:58px;left:<%=x + 630 %>px;top:<%=(i - 1) * 52 + y + 13 %>px;z-index:3;visibility:hidden' />
					<img class='btn_focus_hidden' id='f_artist_<%=i + pageLimit %>' src='images/application/pages/search/song/focus/f_collect.png' style='position:absolute;width:58px;height:58px;left:<%=x + 630 %>px;top:<%=(i - 1) * 52 + y + 13 %>px;z-index:4' />
					<img id='artist_<%=i + pageLimit * 2 %>' src='images/application/pages/search/song/add.png' style='position:absolute;width:58px;height:58px;left:<%=x + 695 %>px;top:<%=(i - 1) * 52 + y + 13 %>px;z-index:3;visibility:hidden' />
					<img class='btn_focus_hidden' id='f_artist_<%=i + pageLimit * 2 %>' src='images/application/pages/search/song/focus/f_add.png' style='position:absolute;width:58px;height:58px;left:<%=x + 695 %>px;top:<%=(i - 1) * 52 + y + 13 %>px;z-index:4' /><%}%>
					<div id='songS'></div>
				</div>
			</div>
			<!-- 优先加载图 -->
			...
			<script type='text/javascript' src='javascript/pages/artist_classic.js?r=<%=Math.random() %>'></script>
