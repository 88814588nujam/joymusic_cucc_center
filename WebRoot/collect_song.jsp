...
<%
	...
	pageIndex = pageIndex == 0 ? 1 : pageIndex; // 因为收藏曲目有空列表问题

	int pageLimit = 10;
	if(ajaxData == 1){ // 列表歌曲
		String tmpUid = StringUtils.isBlank(DoParam.AnalysisAbb("u", request)) ? "" : DoParam.AnalysisAbb("u", request);
		int totalRows = InfoData.getSongsCountByCollect(tmpUid);
		int totalRowsA = InfoData.getSingersCountByCollect(tmpUid);
		Page pages = new Page(totalRows, pageLimit);
		pages.setPageIndex(pageIndex);
		List<Map<String, Object>> gsba = InfoData.getSongsByCollect(pages, tmpUid);
		String retStr = "{'ifNull':false, 'totalRowsA':'" + totalRowsA + "', 'totalRows':'" + totalRows + "', 'pageTotal':'" + pages.getPageTotal() + "', 'pageSum':'" + gsba.size() + "', 'songList':[";
		if(gsba.size() > 0){
	...
	<body onload='start()' bgcolor='<%if(isOnline.equals("y")){%>transparent<%} else{%>#444444<%}%>'>
		...
		<div id='realDis' style='position:absolute;width:<%=pageW %>px;height:<%=pageH %>px;overflow:hidden;display:none'>
			<div id='BigDiv' style='position:absolute;width:<%=pageW %>px;height:<%=pageH  %>px;left:0px;top:0px'>
				...
				<img id='list_100013' src='images/application/pages/search/commonly/r_song.png' style='position:absolute;width:69px;height:37px;left:426px;top:90px;z-index:1' />
				<img class='btn_focus_hidden' id='f_list_100013' src='images/application/pages/search/commonly/focus/f_r_song.png' style='position:absolute;width:69px;height:37px;left:426px;top:90px;z-index:2' />
				<img src='images/application/pages/search/commonly/line.png' style='position:absolute;width:69px;height:1px;left:426px;top:127px;z-index:3'>
				<div id='totalRowsS' style='position:absolute;width:50px;height:30px;left:494px;top:103px;color:#FFAE00'></div>
				<img id='list_100014' src='images/application/pages/search/commonly/r_artist.png' style='position:absolute;width:69px;height:37px;left:555px;top:90px;z-index:1'>
				<img class='btn_focus_hidden' id='f_list_100014' src='images/application/pages/search/commonly/focus/f_r_artist.png' style='position:absolute;width:69px;height:37px;left:555px;top:90px;z-index:2' />
				...
				<div id='rightDiv_mar' style='position:absolute;width:1280px;height:720px;left:25px;top:0px'>
					...
					<img class='btn_focus_hidden' id='f_list_<%=i %>' src='images/application/pages/collect/song/focus/f_play.png' style='position:absolute;width:58px;height:58px;left:<%=x + 563 %>px;top:<%=(i - 1) * 52 + y + 13 %>px;z-index:4' />
					<img id='list_<%=i + pageLimit %>' src='images/application/pages/collect/song/delete.png' style='position:absolute;width:58px;height:58px;left:<%=x + 630 %>px;top:<%=(i - 1) * 52 + y + 13 %>px;z-index:3;visibility:hidden' />
					<img class='btn_focus_hidden' id='f_list_<%=i + pageLimit %>' src='images/application/pages/collect/song/focus/f_delete.png' style='position:absolute;width:58px;height:58px;left:<%=x + 630 %>px;top:<%=(i - 1) * 52 + y + 13 %>px;z-index:4' />
					<img id='list_<%=i + pageLimit * 2 %>' src='images/application/pages/collect/song/add.png' style='position:absolute;width:58px;height:58px;left:<%=x + 695 %>px;top:<%=(i - 1) * 52 + y + 13 %>px;z-index:3;visibility:hidden' />
					<img class='btn_focus_hidden' id='f_list_<%=i + pageLimit * 2 %>' src='images/application/pages/collect/song/focus/f_add.png' style='position:absolute;width:58px;height:58px;left:<%=x + 695 %>px;top:<%=(i - 1) * 52 + y + 13 %>px;z-index:4' /><%}%>
					<div id='songS'></div>
					<div id='recDiv' style='display:none'>
						...
						<img src='images/application/pages/collect/none_s.png' style='position:absolute;width:285px;height:59px;left:675px;top:160px;z-index:10' /><%for(int i = 1; i <= 6; i++){ int y = i <= 3 ? 301 : 453;%>
						...
						<img class='btn_focus_hidden' id='f_list_<%=i + 100006 %>' src='images/application/pages/search/commonly/focus/f_recommend.png' style='position:absolute;width:267px;height:165px;left:<%=((i % 3 == 0 ? 3 : i % 3) - 1) * 254 + 442 %>px;top:<%=y - 11 %>px;z-index:3' /><%}%>
					</div>
				</div>
			</div>
			<script type='text/javascript' src='javascript/pages/collect_song.js?r=<%=Math.random() %>'></script>
