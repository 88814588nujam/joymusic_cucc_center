			...
			function getSongBack(){
				if(xmlhttp.readyState == 4){
					if(xmlhttp.status == 200){
						...
						var totalRowsS = totalRows;
						var totalRowsA = Number(contentJson.totalRowsA);
						...
						if(totalRows == 0){
							...
							if(tempS < 100007 && tempS != 100002) nowFocus = 'f_list_100007';
						...
						if(!multipleLoad){
							...
							} else if(tempF == 100001 || tempF == 100002 || tempF == 100003) $g(nowFocus).setAttribute('class', 'btn_focus_visible');
							else if(tempF == 100013){
								$g('list_100013').style.visibility = 'hidden';
								$g(nowFocus).setAttribute('class', 'btn_focus_visible');
							} else if(tempF > 100006 && tempF < 100013){
			...
			function onkeyOK(){
				if(allowClick){
					...
					if(tempF == 100002){
						...
						var docUrl = dataUrl + '?u=' + userid + '&t=' + hour + '&m=' + max + '&p=' + pageId + '&o=' + opr;
						...
						if(tempF > 0 && tempF < pageLimit + 1){
							...
							operaCollect(dataUrl, sid, userid, tpy, opr, function(json){
								...
								msg.type = getVal('globle', 'preferBubble');
								msg.createMsgArea($g('realDis'));
								var msgStr = '成功删除收藏曲目 [' + songName + ' - ' + artistName + ']';
								msg.sendMsg(msgStr);
								if(pageSum == 1){
									pageIndex--;
									needreload = true;
			...
			function onkeyLeft(){
				if(allowClick){
					...
					$g(nowFocus).setAttribute('class', 'btn_focus_hidden');
					if(tempF == 100001){
						nowFocus = 'f_list_100014';
						$g('list_100014').style.visibility = 'hidden';
						...
						$g('list_100014').style.visibility = 'visible';
			...
			function onkeyUp(){
				if(allowClick){
					...
					if(tempF == 100001) nowFocus = 'f_list_100001';
					...
					else if(tempF == 100007 || tempF == 100008 || tempF == 100009){
						nowFocus = 'f_list_100014';
						$g('list_100014').style.visibility = 'hidden';
						...
						var newIdx = tempF - 1;
						nowFocus = 'f_list_' + newIdx;
					} else if(tempF == 1 || tempF == pageLimit + 1 || tempF == pageLimit * 2 + 1){
						nowFocus = 'f_list_100014';
						$g('list_100014').style.visibility = 'hidden';
					}
					var tempS = getIdx(nowFocus, 'f_list_');
