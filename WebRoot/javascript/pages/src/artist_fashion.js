			...
			function loadSong(){
				allowClick = false;
				var params0 = '[{\'zone\':\'request\',\'key\':\'params\',\'val\':\'d=1&k=' + singerId + '&p=' + pageIndex + '&l=' + pageLimit + '\'}]';
				if(!multipleLoad && pageIndex > 1){
					params0 = '[{\'zone\':\'request\',\'key\':\'params\',\'val\':\'d=1&k=' + singerId + '&p=1&l=' + pageLimit * pageIndex + '\'}]';
				}
				...
				if(xmlhttp.readyState == 4){
					if(xmlhttp.status == 200){
						...
						for(var i = 1; i <= songList.length; i++){
							...
							var artistT = songList[realIdx].artist;
							if(artistT.length > aname.length){
								...
								coDiv.innerText = artistT.replace(',' + aname, '').replace(aname + ',', '');
						...
						if(!multipleLoad){
							multipleLoad = true;
							if(nowFocus.indexOf('f_artist_') < 0){
			...
			function onkeyOK(){
				if(allowClick){
					...
					if(tempF == 100001 || tempF == 100006){
						...
						var nowInfo = '{\'back\':[{\'target\':\'' + pageId + '\'},{\'params\':\'i=' + pageId + '&n=' + nowFocus + '&k=' + singerId + '&b=' + backPos + '&t=' + tipsStat + '&r=' + rightStat + '&p=' + pageIndex + '\'}]}';
						addToBack(nowInfo);
						...
						addToBack(nowInfo);
						toOtrPage(tempF, toPageId, '');
					} else if(tempF == 100004 || tempF == 100007){
						if(tempF == 100007) backPos = 0;
						nowFocus = 'f_artist_100004';
						var opr = 3;
						var dataUrl = 'h2';
					...
					} else if(tempF == 100005 || tempF == 100008){
						nowFocus = 'f_artist_100005';
						preferList = 1;
			...
			function onkeyLeft(){
				if(allowClick){
					var tempF = getIdx(nowFocus, 'f_artist_');
					$g(nowFocus).setAttribute('class', 'btn_focus_hidden');
					if(tempF == 100002 || tempF == 100004) nowFocus = 'f_artist_100001';
			...
			function onkeyUp(){
				if(allowClick){
					...
					else if(tempF == 100004 || tempF == 100005) nowFocus = 'f_artist_100001';
					else if(tempF > 100006 && tempF < 100011){
						var newIdx = tempF - 1;
					...
					movePos('up');
					if(nowFocus.indexOf('f_artist_') < 0) displayBack(lastFocusTmp, nowFocus);
					$g(nowFocus).setAttribute('class', 'btn_focus_visible');
				}
			}
			...
			function toPlayer(){
				...
				var nowInfo = '{\'back\':[{\'target\':\'' + pageId + '\'},{\'params\':\'i=' + pageId + '&n=' + nowFocus + '&k=' + singerId + '&b=' + backPos + '&t=' + tipsStat + '&r=' + rightStat + '&p=' + pageIndex + '\'}]}';
				addToBack(nowInfo);
				var tempF = nowFocus.indexOf('f_play_') > -1 ? 1000 + Number(getIdx(nowFocus, 'f_play_')) : getIdx(nowFocus, 'f_artist_');
				toOtrPage(tempF, toPageId, '');
			}
